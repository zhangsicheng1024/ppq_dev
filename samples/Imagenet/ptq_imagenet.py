...
with ENABLE_CUDA_KERNEL():
    if __name__ == '__main__':
        for model_builder, model_name in (
            ...
            (torchvision.models.swin_t, 'swin_t'),
            # (torchvision.models.swin_s, 'swin_s'),
            # (torchvision.models.swin_b, 'swin_b'),
            # (torchvision.models.swin_v2_t, 'swin_v2_t'),
            # (torchvision.models.swin_v2_s, 'swin_v2_s'),
            # (torchvision.models.swin_v2_b, 'swin_v2_b'),
            
        ):
            print(f'---------------------- PPQ Quantiznination Test Running with {model_name} ----------------------')
            # model = model_builder(pretrained=True).to(CFG_DEVICE)
            # model = model_builder(weights='DEFAULT').to(CFG_DEVICE)
            # model = SwinForImageClassification.from_pretrained("microsoft/swin-tiny-patch4-window7-224").to(CFG_DEVICE)
            print(model)

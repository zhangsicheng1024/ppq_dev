...
with ENABLE_CUDA_KERNEL():
    if __name__ == '__main__':
        for model_builder, model_name in (
            ...
            (torchvision.models.vit_b_16, 'vit_b_16'),
            (torchvision.models.vit_b_32, 'vit_b_32'),
            (torchvision.models.vit_l_16, 'vit_l_16'),
            (torchvision.models.vit_l_32, 'vit_l_32'),
            # (torchvision.models.vit_h_14, 'vit_h_14'),
            ...
            reports_int8 = layerwise_error_analyse(
                graph=ppq_quant_ir_INT8, running_device=CFG_DEVICE, collate_fn=lambda x: x.to(CFG_DEVICE), 
                dataloader=dataloader)  
            reports_fp8 = layerwise_error_analyse(
                graph=ppq_quant_ir_FP8, running_device=CFG_DEVICE, collate_fn=lambda x: x.to(CFG_DEVICE), 
                dataloader=dataloader)  
            ...
            # print("========reports_int8==========", reports_int8)
            # print("========reports_fp8==========", reports_fp8)

            np.save(model_name+'_layer_int8_aligned',reports_int8)
            np.save(model_name+'_layer_fp8',reports_fp8)
            # reports_int8 = np.load(model_name+'_layer_int8_reduce.npy',allow_pickle=True)
            # reports_fp8 = np.load(model_name+'_layer_fp8.npy',allow_pickle=True)
            ...
            # print("========dispatching_table MIX.PY==========", QUANT_SETTING.dispatching_table.dispatchings)
            # 将前十个误差最大的层送上 FP32
            # for op_name, _ in sensitivity[: 1]:
            ...
            # MONITOR THE FINAL PLATFORM ASSIGNMENT
            # print("========dispatching_table FINAL GRAPH==========")
            # for operation in ppq_quant_ir.operations.values():
            #     print(operation.name,operation.platform)
